...
class PAMDeviceLoggingConfigEventHandler(EventHandler[CommunicationResponseEventPayload]):
    """
    ...
    async def handle(self, payload: CommunicationResponseEventPayload):
        print("-------PAMDeviceLoggingConfigEventHandler: Handling event @communication/received_response")
        if payload.opcode != OperationCode.to_int(OperationCode.GET_PAM_DEVICE_LOGGING_CONFIG) and (
                payload.opcode != OperationCode.to_int(OperationCode.SET_PAM_DEVICE_LOGGING_CONFIG)):
            print("PAMDeviceLoggingConfigEventHandler: Operation code is not GET_PAM_DEVICE_LOGGING_CONFIG nor "
                  "SET_PAM_DEVICE_LOGGING_CONFIG, ignoring event")
            return

        if payload.sender_address != Address.PI3:
            print("PAMDeviceLoggingConfigEventHandler: Address is not PI3, ignoring event")
            return

        # Create the response object
        pam_device_logging_config = PAMDeviceLoggingConfigCommunicationResponse(
            payload.response
        )

        print("PAM Device Logging Config Response:")
        print(pam_device_logging_config)

        # Decode the message data, store it in the repository
        self.repository.add_pam_device_logging_config(
            pam_device_logging_config
        )
        if payload.opcode == OperationCode.to_int(OperationCode.GET_PAM_DEVICE_LOGGING_CONFIG):
            self.request_logger_service.resolve_request(
                OperationCode.GET_PAM_DEVICE_LOGGING_CONFIG,
                Address.PI3
            )
            ...
            self.request_logger_service.resolve_request(
                OperationCode.SET_PAM_DEVICE_LOGGING_CONFIG,
                Address.PI3
            )
            # Send a success notification to the client
            await self.notification_service.send_success_notification(
                message="Logging settings set successfully",
            )
