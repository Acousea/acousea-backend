...
class PAMDeviceStreamingConfigEventHandler(EventHandler[CommunicationResponseEventPayload]):
    """
    ...
    async def handle(self, payload: CommunicationResponseEventPayload):
        print("-------PAMDeviceStreamingConfigEventHandler: Handling event @communication/received_response")
        if payload.opcode != OperationCode.to_int(OperationCode.GET_PAM_DEVICE_STREAMING_CONFIG) and payload.opcode != OperationCode.to_int(
                OperationCode.SET_PAM_DEVICE_STREAMING_CONFIG):
            print(
                "PAMDeviceStreamingConfigEventHandler: Operation code is not GET_PAM_DEVICE_STREAMING_CONFIG nor SET_PAM_DEVICE_STREAMING_CONFIG, ignoring event")
            return

        if payload.sender_address != Address.PI3:
            print("PAMDeviceStreamingConfigEventHandler: Address is not PI3, ignoring event")
            return
